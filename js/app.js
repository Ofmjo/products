    ...
    if (tasksInput) {
        ...
        tasksArray.forEach(task => {
            if (!tasks.includes(task)) {
                tasks.push(task);  // Добавляем новый продукт
            }
        });
...
function renderTasks() {
    ...
    tasks.forEach((task, index) => {
        ...
        li.className = "bg-gray-200 p-3 rounded-lg text-lg cursor-pointer hover:bg-gray-300 transition";
        li.textContent = task;
        li.onclick = () => removeTask(index, task);
